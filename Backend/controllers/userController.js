...
exports.registration = async (req, res, next) => {
  ...
  try {
    ...
    if (!firstName || !lastName || !email || !password) {
      ...
      const url = `${process.env.CLIENT_URL}auth/activation/${activationToken}`;
...
exports.updateUser = async (req, res, next) => {
  try {
    ...
    if (user) {
      ...
      user.activeStatus = req.body.activeStatus || user.activeStatus;
...
const createActivationCode = () => {
  return crypto.randomInt(0, 1000000, (err, n) => {
    if (err) throw err;
    //console.log(n);
